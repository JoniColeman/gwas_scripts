...
print(c(i, summary(lm(colnames(DATA)[(i+1)] ~ ., data=DATA[,-(i+1)]))$coefficients[(i+1),4], summary(lm(colnames(DATA)[(i+1)] ~ ., data=DATA[,-(i+1)]))$r.squared - summary(lm(colnames(DATA2)[(i+1)] ~ ., data=DATA2[,-(i+1)]))$r.squared))
}
sink()
