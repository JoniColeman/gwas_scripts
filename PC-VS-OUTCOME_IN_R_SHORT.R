...
print(c(i, summary(lm(PCAPHENO[,104] ~ ., data=DATA))$coefficients[(i+1),4], summary(lm(PCAPHENO[,104] ~ ., data=DATA))$r.squared - summary(lm(PCAPHENO[,104] ~ ., data=DATA2))$r.squared))
}
sink()
